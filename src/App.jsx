...
const SUPABASE_URL = "https://vocyvoqvdpugvytgjtws.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZvY3l2b3F2ZHB1Z3Z5dGdqdHdzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg4Mjc2NTYsImV4cCI6MjA5NDQwMzY1Nn0.tf81rJ-EmAdDdNuBIblxLYQoKf1ugKD8UMtmnG3mvkA";
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --cream: #F5F0E8; --bark: #2C1F0E; --moss: #3D5A3E; --moss-light: #5A7D5B;
    --amber: #C4862A; --amber-light: #E8A84A; --rust: #A63D2F; --mist: #E8EDE8;
    --zen: #4A7C7E; --momentum: #7E4A6A; --elixir: #7E6A2A;
  }
  body { background: var(--cream); color: var(--bark); font-family: 'DM Mono', monospace; min-height: 100vh; }
  .app { display: flex; flex-direction: column; min-height: 100vh; }
  .header { background: var(--bark); padding: 16px 24px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; }
  ...
  .sync-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--moss); display: inline-block; margin-right: 6px; }
  .sync-dot.syncing { background: var(--amber); animation: pulse 1s infinite; }
  .sync-dot.error { background: var(--rust); }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
  .nav { background: var(--bark); border-top: 1px solid #3d2e1a; display: flex; overflow-x: auto; scrollbar-width: none; }
  .nav::-webkit-scrollbar { display: none; }
  .nav-btn { flex: 0 0 auto; padding: 10px 16px; font-family: 'DM Mono', monospace; font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: #8a7a6a; background: none; border: none; cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.15s; white-space: nowrap; }
  ...
  .tank-alert { font-size: 10px; color: var(--rust); margin-top: 3px; }
  .stock-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 14px; }
  .stock-pill { background: var(--mist); border-radius: 10px; padding: 12px 10px; text-align: center; border: 2px solid transparent; }
  ...
  .input-field { width: 100%; padding: 10px 12px; font-family: 'DM Mono', monospace; font-size: 13px; background: var(--mist); border: 1px solid #d8d0c0; border-radius: 8px; color: var(--bark); outline: none; transition: border-color 0.15s; }
  .input-field:focus { border-color: var(--amber); }
  .input-row { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
  select.input-field { appearance: none; cursor: pointer; }
  .btn { padding: 10px 20px; border-radius: 8px; border: none; cursor: pointer; font-family: 'DM Mono', monospace; font-size: 11px; letter-spacing: 1.5px; text-transform: uppercase; transition: all 0.15s; font-weight: 500; }
  ...
  .inner-tab { padding: 6px 14px; border-radius: 20px; border: 1px solid #d8d0c0; font-family: 'DM Mono', monospace; font-size: 10px; letter-spacing: 1px; text-transform: uppercase; cursor: pointer; background: white; color: #7a6a5a; transition: all 0.15s; }
  ...
  .success-toast { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); background: var(--moss); color: white; padding: 10px 20px; border-radius: 8px; font-size: 11px; letter-spacing: 1px; z-index: 999; animation: fadeInUp 0.2s ease; }
  ...
  .loading { text-align: center; padding: 60px 16px; color: #9a8a7a; font-size: 12px; }
  .loading-logo { font-family: 'Fraunces', serif; font-size: 32px; font-weight: 900; color: var(--bark); margin-bottom: 12px; }
`;
...
const CLIENTES = ["Mandala", "Fork", "Cajón", "Otro"];
...
function getWeekLabel() {
  ...
  return `Semana ${mon.getDate()}/${mon.getMonth()+1} – ${fri.getDate()}/${fri.getMonth()+1}`;
}
...
async function dbGet(key) {
  const { data } = await supabase.from("biloba_state").select("value").eq("key", key).single();
  return data?.value ?? null;
}
async function dbSet(key, value) {
  await supabase.from("biloba_state").upsert({ key, value, updated_at: new Date().toISOString() });
}
...
export default function App() {
  ...
  const [bodegaLog, setBodegaLog] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      const [t, b, bl, ins, comp, act] = await Promise.all([
        dbGet("tanks"), dbGet("bodega"), dbGet("bodegaLog"),
        dbGet("insumos"), dbGet("compras"), dbGet("actividades")
      ]);
      setTanks(t || { Zen: 0, Momentum: 0, Elixir: 0 });
      setBodega(b || { Zen: 0, Momentum: 0, Elixir: 0 });
      setBodegaLog(bl || []);
      setInsumos(ins || { te: 0, azucar: 0, botellas: 0, tapas: 0, etiquetas: 0, cajas: 0 });
      setCompras(comp || []);
      setActividades(act || []);
      setLoading(false);
    })();
  ...
  const saveTanks = async (v) => { setTanks(v); setSyncStatus("syncing"); await dbSet("tanks", v); setSyncStatus("ok"); };
  const saveBodega = async (b, log) => { setBodega(b); setBodegaLog(log); setSyncStatus("syncing"); await Promise.all([dbSet("bodega", b), dbSet("bodegaLog", log)]); setSyncStatus("ok"); };
  const saveInsumos = async (v) => { setInsumos(v); setSyncStatus("syncing"); await dbSet("insumos", v); setSyncStatus("ok"); };
  const saveCompras = async (v) => { setCompras(v); setSyncStatus("syncing"); await dbSet("compras", v); setSyncStatus("ok"); };
  const saveActividades = async (v) => { setActividades(v); setSyncStatus("syncing"); await dbSet("actividades", v); setSyncStatus("ok"); };
    ...
    <>
      ...
      <div className="app" style={{ justifyContent: "center", alignItems: "center" }}>
        <div className="loading">
          <div className="loading-logo">Biloba</div>
          Cargando planta...
        </div>
      </div>
    </>
      ...
      <div className="app">
        <header className="header">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span className={`sync-dot ${syncStatus}`} title={syncStatus === "ok" ? "Sincronizado" : syncStatus === "syncing" ? "Guardando..." : "Error"} />
...
function BodegaFria({ bodega, bodegaLog, onSave, toast }) {
  ...
  const [form, setForm] = useState({ sabor: "Zen", cantidad: "", usuario: "Abner", nota: "", fecha: today() });
  const [salForm, setSalForm] = useState({ zen12: 0, zen24: 0, mom12: 0, mom24: 0, eli12: 0, eli24: 0, mix12: 0, mix24: 0, cliente: "Mandala" });
  ...
  const handleEntrada = async () => {
    ...
    const entry = { id: Date.now(), tipo: "entrada", sabor: form.sabor, cantidad: Number(form.cantidad), usuario: form.usuario, nota: form.nota, fecha: form.fecha };
    await onSave(newBodega, [entry, ...bodegaLog]);
    setForm({ sabor: "Zen", cantidad: "", usuario: "Abner", nota: "", fecha: today() });
  ...
  const handleSalida = async () => {
    ...
    const entry = { id: Date.now(), tipo: "salida", detalle: { ...salForm }, botSalida, fecha: today(), usuario: "Fuyen", cliente: salForm.cliente };
    await onSave(newBodega, [entry, ...bodegaLog]);
    setSalForm({ zen12: 0, zen24: 0, mom12: 0, mom24: 0, eli12: 0, eli24: 0, mix12: 0, mix24: 0, cliente: "Mandala" });
  ...
  return (
    <div>
      ...
      {innerTab === "salida" && (
        <div className="card">
          <div className="card-title">Armar despacho</div>
          <div className="input-group">
            <label className="input-label">Cliente</label>
            <select className="input-field" value={salForm.cliente} onChange={e => setSalForm({ ...salForm, cliente: e.target.value })}>
          ...
          <div style={{ marginTop: 12, padding: 10, background: "var(--mist)", borderRadius: 8, fontSize: 11 }}>
            {SABORES.map(s => {
              ...
              return <div key={s} style={{ marginBottom: 2 }}>{s}: <strong>{n}</strong> (stock: {bodega?.[s]||0})</div>;
            })}
          </div>
              ...
              <div key={e.id} className="list-item">
                <div className="list-item-main">
                  {e.tipo === "entrada"
                    ...
                    : <><span className="badge badge-general">Despacho</span> {e.cliente && <span style={{marginLeft:4,fontSize:10,color:"var(--moss)"}}>{e.cliente}</span>} {SABORES.map(s => `${s}: ${e.botSalida?.[s]||0}`).join(" · ")}</>
                  }
...
function Tanques({ tanks, onSave, toast }) {
  ...
  const handleSave = async () => {
    ...
    await onSave({ ...tanks, [form.sabor]: newVal });
...
function Insumos({ insumos, onSave, toast }) {
  ...
  const handleSave = async () => {
    ...
    await onSave({ ...insumos, [form.id]: newVal });
...
function Compras({ compras, onSave, insumos, onSaveInsumos, toast }) {
  ...
  const handleSave = async () => {
    ...
    await onSave(newCompras);
    if (form.categoria === "Insumo" && form.insumoId && form.insumoCant) {
      await onSaveInsumos({ ...insumos, [form.insumoId]: (insumos[form.insumoId]||0) + Number(form.insumoCant) });
    }
...
function Actividades({ actividades, onSave, toast }) {
  ...
  const handleSave = async () => {
    ...
    await onSave([{ id: Date.now(), ...form }, ...actividades]);
    ...
    <div>
      ...
      {mode === "mensual" && <>
        <div className="report-block">
          ...
          <div className="report-stat"><span className="report-stat-label">Total</span><span className="report-stat-value">{Object.values(monthEmb).reduce((a,b)=>a+b,0)} botellas</span></div>
          ...
          {monthDespachos.length === 0
            ...
            : monthDespachos.map((d,i) => (
                <div key={d.id} style={{marginBottom:8}}>
                  <div style={{fontSize:10,color:"var(--amber-light)",marginBottom:3}}>Despacho {i+1} — {fmtDate(d.fecha)}{d.cliente ? ` · ${d.cliente}` : ""}</div>
